...
impl Engine for CEngine {
    ...
    fn shader_spv(&self, shader: Shader) -> &[u8] {
        // Phase 8 M11 (task plan D8): the blobs are compiled and embedded by
        // build.rs; every `Shader` name is a job in the xtask table.
        crate::shaders::spv(shader.name().to_str().expect("ASCII shader name"))
            .unwrap_or_else(|| panic!("no SPIR-V embedded for {:?}", shader.name()))
    }
