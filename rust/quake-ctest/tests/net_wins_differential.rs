//! Differential test: the pure address functions of the Rust Winsock
//! landriver personality (`quake_net::udp::*_wins` plus the helpers it
//! shares with the unix driver) vs the C originals in `Quake/net_wins.c`
//! (compiled as `c_ref_*`, Windows only). Phase 9 M2.
//!
//! Scope, precisely: WINS_AddrToString (both `masked` arms -- unlike
//! net_udp.c this driver honours the flag), WINIPv4_StringToAddr,
//! WINS_AddrCompare, WINS_Get/SetSocketPort and PartialIPAddress (via
//! WINIPv4_GetAddrFromName's leading-digit branch). The socket/resolver
//! halves are exercised end-to-end by the engine-level interop matrix
//! (identical live sessions over the Rust and C drivers).
//!
//! The Windows SDK defines IPPROTO_IPV6 as an enumerator, not a macro, so
//! on MSVC/clang-cl net_wins.c compiles without its AF_INET6 branches (and
//! net_win.c without the IPv6 landriver, so no AF_INET6 sockaddr ever
//! reaches them in the engine). The probe reports which way this
//! toolchain went; without IPv6 the AF_INET6 inputs are skipped.
...
extern "C" {
    fn c_ref_WINS_AddrToString(addr: *mut QSockAddr, masked: bool) -> *const c_char;
    fn c_ref_WINIPv4_StringToAddr(string: *const c_char, addr: *mut QSockAddr) -> c_int;
    fn c_ref_WINS_AddrCompare(a: *mut QSockAddr, b: *mut QSockAddr) -> c_int;
    fn c_ref_WINS_GetSocketPort(addr: *mut QSockAddr) -> c_int;
    fn c_ref_WINS_SetSocketPort(addr: *mut QSockAddr, port: c_int) -> c_int;
    fn c_ref_WINIPv4_GetAddrFromName(name: *const c_char, addr: *mut QSockAddr) -> c_int;
    static mut c_ref_net_hostport: c_int;
    fn ctest_abi_net_lookup(key: *const c_char) -> usize;
}

fn oracle_has_ipv6() -> bool {
    let key = c"const.IPPROTO_IPV6_defined";
    // SAFETY: the probe only strcmp's the key against a compile-time table
    let v = unsafe { ctest_abi_net_lookup(key.as_ptr()) };
    assert_ne!(
        v,
        usize::MAX,
        "IPPROTO_IPV6_defined missing from the C probe"
    );
    v != 0
}

fn is_v6(a: &QSockAddr) -> bool {
    udp::family(a) == udp::AF_INET6
}
...
fn c_addr_to_string(a: &QSockAddr, masked: bool) -> String {
    let mut c = *a;
    // SAFETY: serialized by TEST_LOCK; the static return buffer is copied out
    unsafe {
        CStr::from_ptr(c_ref_WINS_AddrToString(&mut c, masked))
            .to_string_lossy()
            .into_owned()
    }
}
...
fn addr_to_string_matches() {
    ...
    let mut cases = vec![
        ...
        QSockAddr::zeroed(), // family 0: net_wins.c formats it as IPv4
        {
            let mut a = mk_v4([1, 2, 3, 4], 9);
    ...
    let v6 = oracle_has_ipv6();
    for (i, a) in cases.iter().enumerate().filter(|(_, a)| v6 || !is_v6(a)) {
        for masked in [false, true] {
            assert_eq!(
                c_addr_to_string(a, masked),
                udp::addr_to_string_wins(a, masked),
                "case {i} masked {masked}"
            );
        }
    }
}
...
fn string_to_addr_matches() {
    ...
    for s in &cases {
        ...
        let cret = unsafe { c_ref_WINIPv4_StringToAddr(cs.as_ptr().cast(), &mut ca) };
...
fn addr_compare_and_ports_match() {
    ...
    for _ in 0..60 {
        let a = match rng.next() % 4 {
            ...
            _ => {
                // unknown families: net_wins.c compares them as IPv4, so
                // give them distinguishable v4 bytes
                let mut a = mk_v4([10, 0, 0, (rng.next() % 2) as u8], (rng.next() % 2) as u16);
                udp::set_family(&mut a, (rng.next() % 5) as i32);
                a
            }
        };
        if oracle_has_ipv6() || !is_v6(&a) {
            pool.push(a);
        }
    }
    for x in &pool {
        for y in &pool {
            ...
            let cret = unsafe { c_ref_WINS_AddrCompare(&mut cx, &mut cy) };
            assert_eq!(cret, udp::addr_compare_wins(x, y));
        }
        let mut cx = *x;
        // SAFETY: serialized by TEST_LOCK
        let cport = unsafe { c_ref_WINS_GetSocketPort(&mut cx) };
        assert_eq!(cport, udp::get_socket_port_wins(x));

        let mut cs = *x;
        let mut rs = *x;
        let p = (rng.next() % 70000) as c_int;
        // SAFETY: serialized by TEST_LOCK
        let cret = unsafe { c_ref_WINS_SetSocketPort(&mut cs, p) };
        let rret = udp::set_socket_port_wins(&mut rs, p);
        assert_eq!(cret, rret);
...
fn partial_ip_address_matches() {
    let _l = lock();
    // via WINIPv4_GetAddrFromName's leading-digit branch. The c_ref myAddrv4
    // static is 0 (WINIPv4_Init never runs in tests); the Rust side gets the
...
fn host_port_guards_match() {
    ...
    let cret = unsafe { c_ref_WINIPv4_GetAddrFromName(cs.as_ptr().cast(), &mut ca) };
    assert_eq!(cret, -1);
    assert!(udp::split_host_port(long.as_bytes(), udp::MAXHOSTNAMELEN).is_none());

    // net_wins.c only: a host part with a colon of its own is refused before
    // the resolver too (net_wins.rs mirrors this after split_host_port)
    let multi = "fe80::1:26000";
    let mut cs = multi.as_bytes().to_vec();
    cs.push(0);
    // SAFETY: serialized by TEST_LOCK
    let cret = unsafe { c_ref_WINIPv4_GetAddrFromName(cs.as_ptr().cast(), &mut ca) };
    assert_eq!(cret, -1);
