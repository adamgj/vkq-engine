...
static const ctest_abi_entry_t ctest_abi_render_table[] = {
	...
	/* Phase 8 M4: gltexture_t is the Rust texture manager's list node, read
	 * by every C renderer file through the C view; the layout is the
	 * prelude's copy of gl_texmgr.h, engine-header-checked by the
	 * COMPILE_TIME_ASSERTs in Quake/gl_texmgr_glue.c. */
	SZ ("gltexture_t", gltexture_t),
	OFF ("gltexture_t", gltexture_t, next),
	OFF ("gltexture_t", gltexture_t, owner),
	OFF ("gltexture_t", gltexture_t, name),
	OFF ("gltexture_t", gltexture_t, path_id),
	OFF ("gltexture_t", gltexture_t, width),
	OFF ("gltexture_t", gltexture_t, height),
	OFF ("gltexture_t", gltexture_t, flags),
	OFF ("gltexture_t", gltexture_t, source_file),
	OFF ("gltexture_t", gltexture_t, source_offset),
	OFF ("gltexture_t", gltexture_t, source_format),
	OFF ("gltexture_t", gltexture_t, source_width),
	OFF ("gltexture_t", gltexture_t, source_height),
	OFF ("gltexture_t", gltexture_t, source_crc),
	OFF ("gltexture_t", gltexture_t, shirt),
	OFF ("gltexture_t", gltexture_t, pants),
	OFF ("gltexture_t", gltexture_t, image),
	OFF ("gltexture_t", gltexture_t, image_view),
	OFF ("gltexture_t", gltexture_t, target_image_view),
	OFF ("gltexture_t", gltexture_t, allocation),
	OFF ("gltexture_t", gltexture_t, descriptor_set),
	OFF ("gltexture_t", gltexture_t, frame_buffer),
	OFF ("gltexture_t", gltexture_t, storage_descriptor_set),
	SZ ("enum srcformat", enum srcformat),
	{"const.SRC_INDEXED", SRC_INDEXED},
	{"const.SRC_LIGHTMAP", SRC_LIGHTMAP},
	{"const.SRC_RGBA", SRC_RGBA},
	{"const.SRC_SURF_INDICES", SRC_SURF_INDICES},
	{"const.SRC_RGBA_CUBEMAP", SRC_RGBA_CUBEMAP},
	{"const.SRC_INDEXED_PALETTE", SRC_INDEXED_PALETTE},
	{"const.TEXPREF_MIPMAP", TEXPREF_MIPMAP},
	{"const.TEXPREF_LINEAR", TEXPREF_LINEAR},
	{"const.TEXPREF_NEAREST", TEXPREF_NEAREST},
	{"const.TEXPREF_ALPHA", TEXPREF_ALPHA},
	{"const.TEXPREF_PAD", TEXPREF_PAD},
	{"const.TEXPREF_PERSIST", TEXPREF_PERSIST},
	{"const.TEXPREF_OVERWRITE", TEXPREF_OVERWRITE},
	{"const.TEXPREF_NOPICMIP", TEXPREF_NOPICMIP},
	{"const.TEXPREF_FULLBRIGHT", TEXPREF_FULLBRIGHT},
	{"const.TEXPREF_NOBRIGHT", TEXPREF_NOBRIGHT},
	{"const.TEXPREF_CONCHARS", TEXPREF_CONCHARS},
	...
	/* Phase 8 M9: cb_context_t (the Rust frame graph owns the
	 * secondary_cb_contexts array and every draw entry point takes one by
	 * pointer) is now the prelude's verbatim copy; the M8 mirrors the C glue
	 * reads by layout (lerpdata_t, struct lightmap_s and its members) get
	 * their probe rows here (review finding F3 on PR #38). The engine-header
	 * check for all of them is the COMPILE_TIME_ASSERT block in
	 * Quake/gl_rmain_glue.c. */
	SZ ("cb_context_t", cb_context_t),
	OFF ("cb_context_t", cb_context_t, cb),
	OFF ("cb_context_t", cb_context_t, current_canvas),
	OFF ("cb_context_t", cb_context_t, render_pass),
	OFF ("cb_context_t", cb_context_t, render_pass_index),
	OFF ("cb_context_t", cb_context_t, subpass),
	OFF ("cb_context_t", cb_context_t, current_pipeline),
	OFF ("cb_context_t", cb_context_t, vbo_indices),
	OFF ("cb_context_t", cb_context_t, num_vbo_indices),
	{"const.MAX_BATCH_SIZE", MAX_BATCH_SIZE},
	SZ ("lerpdata_t", lerpdata_t),
	OFF ("lerpdata_t", lerpdata_t, pose1),
	OFF ("lerpdata_t", lerpdata_t, pose2),
	OFF ("lerpdata_t", lerpdata_t, blend),
	OFF ("lerpdata_t", lerpdata_t, origin),
	OFF ("lerpdata_t", lerpdata_t, angles),
	SZ ("lm_compute_workgroup_bounds_t", lm_compute_workgroup_bounds_t),
	OFF ("lm_compute_workgroup_bounds_t", lm_compute_workgroup_bounds_t, mins),
	OFF ("lm_compute_workgroup_bounds_t", lm_compute_workgroup_bounds_t, maxs),
	OFF ("lm_compute_workgroup_bounds_t", lm_compute_workgroup_bounds_t, submodel),
	SZ ("glRect_t", glRect_t),
	OFF ("glRect_t", glRect_t, l),
	OFF ("glRect_t", glRect_t, t),
	OFF ("glRect_t", glRect_t, w),
	OFF ("glRect_t", glRect_t, h),
	SZ ("glMaxUsed_t", glMaxUsed_t),
	OFF ("glMaxUsed_t", glMaxUsed_t, w),
	OFF ("glMaxUsed_t", glMaxUsed_t, h),
	SZ ("struct lightmap_s", struct lightmap_s),
	OFF ("struct lightmap_s", struct lightmap_s, texture),
	OFF ("struct lightmap_s", struct lightmap_s, surface_indices_texture),
	OFF ("struct lightmap_s", struct lightmap_s, lightstyle_textures),
	OFF ("struct lightmap_s", struct lightmap_s, descriptor_set),
	OFF ("struct lightmap_s", struct lightmap_s, modified),
	OFF ("struct lightmap_s", struct lightmap_s, workgroup_bounds_buffer),
	OFF ("struct lightmap_s", struct lightmap_s, rectchange),
	OFF ("struct lightmap_s", struct lightmap_s, lightstyle_rectused),
	OFF ("struct lightmap_s", struct lightmap_s, global_bounds),
	OFF ("struct lightmap_s", struct lightmap_s, active_dlights),
	OFF ("struct lightmap_s", struct lightmap_s, block_has_submodels),
	OFF ("struct lightmap_s", struct lightmap_s, num_used_lightstyles),
	OFF ("struct lightmap_s", struct lightmap_s, used_lightstyles),
	OFF ("struct lightmap_s", struct lightmap_s, cached_light),
	OFF ("struct lightmap_s", struct lightmap_s, cached_framecount),
	OFF ("struct lightmap_s", struct lightmap_s, data),
	OFF ("struct lightmap_s", struct lightmap_s, lightstyle_data),
	OFF ("struct lightmap_s", struct lightmap_s, surface_indices),
	OFF ("struct lightmap_s", struct lightmap_s, workgroup_bounds),
	{"const.LMBLOCK_WIDTH", LMBLOCK_WIDTH},
	{"const.LMBLOCK_HEIGHT", LMBLOCK_HEIGHT},
	{"const.LM_CULL_BLOCK_W", LM_CULL_BLOCK_W},
	{"const.LM_CULL_BLOCK_H", LM_CULL_BLOCK_H},
	{"const.TASKS_MAX_WORKERS", TASKS_MAX_WORKERS},
	{"const.MAX_LIGHTSTYLES", MAX_LIGHTSTYLES},
};
