...
`unsafe` tokens in `src/**/*.rs` with comment lines excluded (the method the
ROADMAP's Phase 8 block recorded), a breakdown by token kind, the crate-level
...
ALLOW_INNER_RE = re.compile(r"^\s*#!\[allow\(unsafe_code(?:,[^)]*)?\)\]", re.M)
ALLOW_OUTER_RE = re.compile(r"^\s*#\[allow\(unsafe_code(?:,[^)]*)?\)\]\s*(?:pub(?:\([a-z]+\))?\s+)?mod\s+([A-Za-z_][A-Za-z0-9_]*)", re.M | re.S)
...
def scan_crate(name):
    src = os.path.join(ROOT, "rust", name, "src")
    ...
    for path in files:
        ...
        if rel in ("src/lib.rs", "src/main.rs"):
            if FORBID_RE.search(text):
                ...
                attr = "deny"
        if ALLOW_INNER_RE.search(text):
            allow_modules.append(rel)
        for m in ALLOW_OUTER_RE.finditer(text):
            allow_modules.append("%s (mod %s)" % (rel, m.group(1)))
...
def render(crates):
    ...
    w("`unsafe` tokens per workspace crate over `src/**/*.rs`, comment lines")
    w("excluded (the count the ROADMAP's Phase 8 block first recorded by hand),")
