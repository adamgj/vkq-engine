...
- **`-nomouse`** — the initial mouse-grab delta would rotate the view. **`-nosound`** — a duplicate-sound start consumes `COM_Rand` only when the mixer has not yet advanced the channel, i.e. depending on the real audio clock, which shifted particle lifetimes by a frame; sound parity is `-sndhash`'s job. **`0 con_notifytime 0`** — the "Wrote …" notify for a screenshot is printed from the asynchronous end-rendering task, so it landed on a wall-clock frame and put a timestamped filename into the next capture.
