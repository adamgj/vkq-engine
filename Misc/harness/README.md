...
| `-parthash` | additionally fold the classic particle simulator's active list into the client hash. Opt-in because it changes the hash chain, and so every committed golden on every platform; the goldens are generated without it, and it is used only by golden-free `--stability` legs (`run_corpus.py --stability --extra-args=-parthash`) |
| `-renderhash <file>` | windowed runs only (Phase 8, [ADR-015](../../docs/rust-migration/adr/ADR-015-renderer-port-then-modernize.md)): a per-frame chain over the renderer's cull decisions and draw-call structure — every `vkCmdDraw*` issued (pipeline, vertex/index/instance counts, first index/vertex/instance) captured by trampolines installed on the engine's own dispatch pointers, kept as one chain per command-buffer context, folded per frame with the world `surfvis` bitset and the entity cull fold. Forces the same fixed timestep and RNG seed as `-demohash`. Writes `R <frame> <chain>` lines, a `P <frame> <count> <chain>` line on the first frame after pipelines were created (every `vkCreate*Pipelines` name in creation order, so a ported pipeline builder is compared before its first draw; `<count>` and the chain are cumulative over the run, so a `vid_restart` that re-creates every pipeline yields e.g. `P 1 660` after `P 0 330`, not a repeat of `330`) and a final `END`; never touches the state-hash chain. `render_corpus.py` counts every line of the file, `P` and `END` included, in its "over N lines" summaries |
| `-renderhashdetail` | with `-renderhash`: additionally write a `D <frame> sv=… c<i>=… ent=… en=… cull=… n=…` line per frame with the per-context chains, so a divergence can be localised to one context before the fold hides it |
...
- `run_corpus.py` — drive [corpus.json](corpus.json): `--generate` / `--check` goldens, `--stability` (run-twice), or `--compare <other-build>` (two builds or two argument sets, e.g. the worker-count perturbation leg; needs no goldens)
- `render_corpus.py` — the windowed renderer gate over [render_corpus.json](render_corpus.json): `--stability` (run-to-run, sets SSIM floors), `--compare <other-build>` (two builds: identical `-renderhash` chains, screenshot SSIM above the per-platform threshold, optional `--validation`), `--timedemo` (median fps, `--timedemo-compare` within `--timedemo-tolerance`). See the render corpus section below
...
- `interop_matrix.py` — client x server localhost matrix across the negotiable protocol cells (`Base-/FTE+` 15/666/999; optional `--ipv6` leg); `--combos` selects the build pairs (the two-build interface remains, CI runs `--combos R/R` with one binary on both sides since the C oracle was deleted). `--soak` runs long sessions instead (see below). Runs in CI on the Linux harness job (Phase 5 M8) and, since Phase 9 M2, on the Windows harness job over the Rust Winsock landriver — the UDP runtime leg the `net_wins.c` port was gated on; IPv4 only there, since there is no IPv6 landriver on MSVC/clang-cl until the Phase 10 M9 flip
...
- `trace_diff.py` — the ADR-019 gate-3 consumer: same headless scenario on two `-Dtrace=true` builds, every VM record compared in order, with a minimum-record floor (demo playback starts no server and so emits **zero** progs records — the trace scenarios are maps)
...
Current coverage: `darwin-arm64` (full local tier, generated before Phase 7) and `windows-x86_64` (generated at Phase 7 M11). The three pusher entries, `e1m1-long`, `save-e2m1` and `music-wav` have goldens on `windows-x86_64` only; `darwin-arm64` predates them and `run_corpus.py --check` skips a missing golden without failing, so a green `--check` on macOS is not evidence about those entries. Windows CI runs `--check` (plain and `--sndhash`) against the release build; the `--stability` steps stay because they also cover the registered-tier entries CI has no data for. `linux-x86_64` goldens come from the `generate-goldens-linux` `workflow_dispatch` job in `build-linux.yml`, which rebuilds the C oracle from tag `c-reference/final` and uploads the set for committing (recorded ROADMAP amendment, Phase 9 deletion PR); until that set is committed the Linux `--check` steps skip every entry and Linux CI's gate is run-twice stability plus the single-build byte differs, which need no goldens.

## Render corpus (Phase 8)

[render_corpus.json](render_corpus.json) drives `render_corpus.py`, the only gate that executes renderer code: the headless corpus never does (`-headless` sets `no_rendering`), so renderer parity needs a **windowed** engine. Each entry starts a demo or map through `-harnesscmds`, takes `screenshot tga` at fixed frames and quits; `timedemo` entries measure fps instead. Three checks, run-to-run in CI since the C oracle was deleted (two-build `--compare` remains for local A/B runs):
...
- **Validation** — `--validation` passes `-validation` (`VK_LAYER_KHRONOS_validation`) and fails on any layer message in the condebug log, and also fails if the run never printed `Using VK_LAYER_KHRONOS_validation`: the engine only requests the layer in a `_DEBUG` (`-Dbuildtype=debug`) build, so pass debug builds (`build-rs-debug`). Local gate only: the validation layer is not part of the SDK subset Linux CI installs.

What the harness had to control to make a windowed run deterministic (all inside `render_corpus.py`):

- **Entity contexts are folded commutatively.** `R_DrawEntitiesOnList` hands each entity to whichever recording task asks next, so both the order and the partition across the entity command buffers vary run to run; their draws are hashed individually and summed, every other context stays an ordered chain.
- **Draws after `SCR_DrawDone` fold into the next frame's line** (the GUI and post-process contexts record after the draw-done task); the chain is still exact, only the frame attribution shifts by one.
- **`-nomouse`** — the initial mouse-grab delta would rotate the view. **`-nosound`** — a duplicate-sound start consumes `COM_Rand` only when the mixer has not yet advanced the channel, i.e. depending on the real audio clock, which shifted particle lifetimes by a frame; sound parity is `-sndhash`'s job. **`0 con_notifytime -1`** — the "Wrote …" notify for a screenshot is printed from the asynchronous end-rendering task, so it landed on a wall-clock frame and put a timestamped filename into the next capture. `0` still left a one-frame window (`con_times` is a float, `realtime` a double, so the stored time rounds above `realtime` about half the time), which showed up as a C-vs-C `-renderhash` flake in the GUI chain on screenshot frames during M2 verification; `-1` closes it, so notify lines are never part of the hash.
- **Screenshot frame attribution is an engine fix (M1, `gl_vidsdl.c`).** `take_screenshot` used to be consumed by whichever end-rendering task ran next — sometimes the previous frame's, still in flight — and that task also cleared a request made after its check, occasionally dropping the screenshot entirely. `SCR_ScreenShot_f` and `GL_EndRendering` now hand the request over on the main thread inside the frame's task parameters, so a screenshot always captures the frame issued after the command. The script keeps one retry for a run that exits cleanly with no output, reported when it triggers.
- **`timedemo` entries run without `-renderhash`**, because the flag forces the fixed 1/72s timestep and the fps would be meaningless. They are also exempt from the `sys_ticrate` pacing floor `main_sdl.c` applies to non-fixed-dt harness runs (a two-process networking safeguard; a timedemo has no peer), which otherwise capped every build at 40 fps. The threshold is the C build's median on the same machine (`--timedemo-compare`, default 10 %); CI lavapipe numbers are recorded, not gated.
