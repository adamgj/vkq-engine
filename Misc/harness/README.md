...
- `render_corpus.py` — the windowed renderer gate over [render_corpus.json](render_corpus.json): `--stability` (C-vs-C run-to-run, sets SSIM floors), `--compare <other-build>` (mixed-vs-C: identical `-renderhash` chains, screenshot SSIM above the per-platform threshold, optional `--validation`), `--timedemo` (median fps, `--timedemo-compare` within `--timedemo-tolerance`). See the render corpus section below
...
- `interop_matrix.py` — 4-way C/Rust client x server localhost matrix across the negotiable protocol cells (`Base-/FTE+` 15/666/999; optional `--ipv6` leg). `--soak` runs long sessions instead (see below). Runs in CI on the Linux harness job (Phase 5 M8) and, since Phase 9 M2, on the Windows harness job over the Rust Winsock landriver — the UDP runtime leg the `net_wins.c` port was gated on; IPv4 only there, since the C oracle ships no IPv6 landriver on MSVC/clang-cl (`Quake/net_win.c`)
