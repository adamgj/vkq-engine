...
| T1.3 | Soak driver: `interop_matrix.py --soak` — frame-count target, periodic server state-hash exchange, desync = liveness/traffic-profile deviation vs same-protocol C/C reference OR Host_Error/protocol error/drop (criterion amended 2026-08-29, see log; the hash stream is a diagnostic, not pass/fail); on failure auto-dump hash window + repro demo (see Soak definition below) | Sonnet high (design reviewed by main session) |
...
| T1.7 | Three read-only audits, parallel: (a) ADR-005 specifier audit of host_cmd.c savegame writer + `Host_WriteConfiguration` + `Key_WriteBindings`; (b) cl_parse.c inventory of all `Host_*` sites with per-svc context (planned as 39; the audit found **41**, see log); (c) cvar/cmd external-linkage surface | Haiku low ×3 |
...
- T7.3 (**Opus high**): cl_parse.c — svc dispatch, statusization of all 41 `Host_*` sites (input: T1.7b inventory; 39 was the pre-audit estimate), CSQC load chain. The phase's largest ADR-009 surface.
...
5. cl_parse.c has 41 `Host_*` sites (T1.7b; 39 was the pre-audit estimate) — the phase's largest ADR-009 statusization surface; per-svc status returns; post-guard invariant per message.
...
19. Live captures are timing-nondeterministic — soak pass criterion is liveness + traffic profile, never byte identity and — since the 2026-08-29 amendment — never exact state-hash identity either.
...
- **2026-08-29 T1.3 soak criterion replaced: exact-hash → tolerant (user-approved plan amendment).** The committed T1.3 row and invariant 19 specified an exact state-hash criterion ("desync = hash mismatch vs same-seed C/C reference"). Functional validation disproved it as a *gate*: a dedicated server under `-demohash` is fixed-dt and deterministic given its input, but which server frame a live UDP datagram from a separately-launched client lands on is not, and a one-frame shift in when a `clc_move` is applied forks two otherwise-identical simulations permanently. Repeat runs of the identical binary confirm it — the same C/C cell reports `hash-identical` on some runs and `hash forks @128` on others. This also means no injected fault landing after the natural fork point could ever be *attributed* to the injection, so the exact-hash model could neither pass cleanly nor prove itself (ADR-019). Per-frame simulation equivalence is already gated exactly, and correctly, by `run_corpus.py`'s single-process fixed-dt goldens; the soak's distinct job is what only a live two-process session can show. **User asked and approved 2026-08-29:** the pass criterion is now (a) no `Host_Error`/`Sys_Error`/crash/timeout/unexpected exit on either side, (b) expected negotiated protocol, (c) **liveness** — the cell's server reached every checkpoint frame its protocol's reference reached (a missing checkpoint = died/dropped/exited early, the actual soak failure mode), and (d) **traffic profile** within the tolerance the non-soak matrix gate already uses. The hash stream is still captured and its first diverging checkpoint reported as a *diagnostic*, never pass/fail. Fault injection was retargeted to suit: `sv_gravity 400` is a pure simulation skew invisible to a liveness/traffic gate, so the default is now `net_messagetimeout 0`, which makes the server's own timeout check (`net_dgrm.c:204`) fire and drop the client — still a config skew, never a byte hack. The T1.3 row and invariant 19 above are updated to match; `interop_matrix.py`'s module docstring is the implementation reference.
