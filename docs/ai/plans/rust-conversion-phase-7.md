...
- **2026-08-29 M1 gate evidence (all measured, Windows x64, this checkout).**
  ...
  - `physics_matrix.py` 12-cell CI trim × 7 entries, C-vs-C: **72 ok, 12 skipped** (`gamedir-switch`, no local `hipnotic/pak0.pak`), **0 failed**; red-tested on the elevators axis via `e1m1-plat-crush` (hash chains fork at `F 56` between `sv_gameplayfix_elevators` 0 and 3, same-cvar rerun byte-identical). Lerps axis still vacuous per the KNOWN GAP above.
