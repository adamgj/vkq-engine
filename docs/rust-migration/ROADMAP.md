...
- CI: extend `.github/workflows/` with the mixed-build matrix, `cargo audit`, `cargo deny` (license allowlist + advisory gate), clippy, rustfmt.
...
Small, dependency-free code with outsized compatibility leverage. Crates: `quake-util`, `quake-math`, parts of `quake-types`.
