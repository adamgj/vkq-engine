...
- [x] PCM-hash parity on the demo corpus incl. a committed synthetic-WAV music entry (`music-wav`; `run_corpus.py --sndhash` goldens + CI compare legs); per-codec decode comparisons ran engine-level against the same C decoder libraries (WAV both natively and in CI; vorbis + flac locally over registered-tier data — lossy fixtures need encoders CI lacks).
