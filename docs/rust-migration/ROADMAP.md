...
**CI caveat (load-bearing):** this branch has not been pushed, so the newest CI run on it is still M2. **M3–M11 have never been through a Linux, macOS, MinGW or arm64 leg** — and both M5 and M9e turned up real Linux-only duplicate-symbol defects that only CI found, so this is a live risk rather than a formality.
