...
**Status (2026-08-27):** M1–M3 of the task plan implemented. **The interpreter is flipped**: under `-Duse_rust_progs` the engine runs the Rust VM and `pr_exec.c` is no longer compiled. ADR-019 gate 3 is live — `trace_diff.py` reports byte-identical instruction traces C-vs-Rust over e1m1/e1m2/e2m1/e3m1/e4m1 (117k–308k records each), and the full corpus `--check`/`--compare` and `save_diff` are clean. M2 added the ADR-006 edict arena, the FIFO free list and the progs string table (18 differential tests + a Miri strict-provenance model of a builtin re-entering the VM). `Host_Guard`/`Host_Reraise` (ADR-009 rule 3) were built at M3 so a builtin's `Host_Error` cannot `longjmp` across a Rust frame. Earlier: M1 of the task plan implemented — the `quake-types::progs` ABI mirrors (incl. all 47 `qcvm_t` offsets and the `DEBUG`/`_DEBUG` `edict_t` fork) with the `progs_abi.rs` per-platform gate; the `use_rust_progs` meson switch + `progs` cargo feature + the `build-rs-cprogs` oracle config; and **`scripts/harness/trace_diff.py`**, the ADR-019 gate-3 consumer Phase 0 deliberately left unbuilt. Verified finding: demo playback never starts a server and emits **zero** progs trace records, so the trace oracle's scenarios are maps and the differ carries a minimum-record floor.
