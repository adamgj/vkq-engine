...
**Status (2026-09-11):** M1-M6 of the task plan implemented. M1 (gates first, no porting): the `use_rust_tasks`/`use_rust_render` switches with their C-oracle CI legs on all three OSes, the ADR-003 amendment for the four planned crates, the windowed render harness (`scripts/harness/render_corpus.py`, `-renderhash`, SSIM/validation/timedemo modes) with C-vs-C stability proven exact on the Windows dev box and wired to Linux CI on lavapipe, and the `loom`/`tsan` job shapes in `rust.yml`. M2 (`quake-tasks`, [ADR-016](adr/ADR-016-task-system.md)): the scheduler is an `unsafe`-free generic crate (256-slot epoch-handle table, dependency/indexed state machine, `crossbeam-deque` in the staticlib and a mutex queue under `cfg(loom)`; 23 unit tests, 5 loom models), the C ABI lives in `quake-capi/src/tasks.rs`, `tasks.c` is swapped for `tasks_glue.c` (the `_DEBUG` `test_tasks` command only) under `-Duse_rust_tasks`, and `crossbeam-deque`/`loom` are adopted in ADR-003. Verified on the Windows dev box: headless corpus compares identical against `build-c` and `build-rs-ctasks` (including the 1-vs-8 worker cell), `test_tasks` in a debug build, and the render corpus (C renderer over Rust tasks) with identical `-renderhash` chains, SSIM 1.0, validation clean and `timedemo` within tolerance; TSan under WSL and in CI (the `tsan` job overrides the `panic = "abort"` profile to unwind, which `-Zbuild-std` needs). Amendments recorded in the plan: the two crate-level switches stay until Phase 9 like Phase 7's (the C renderer remains the SSIM/`-renderhash` oracle, so "switches removed" moves to Phase 9); sub-slices land as commits under them rather than behind switches of their own; `atomics.h` retires with the last C renderer file, not with `tasks.c` (ADR-016 amendment); a debug-build MSVC link defect in the Phase 7 host shim (`printf` from `quake_rs_host_shutdown`) is recorded for a separate fix; the render harness prelude moved to `con_notifytime -1` after a C-vs-C notify-line flake surfaced in M2 verification; and `-renderhash` names draws by `VkPipeline` handle, which the dev-box driver aliases across differently named pipelines, so an unreproduced C-only outlier run is recorded as risk RA14 with the fix scheduled for the M5 pipeline port (root-caused and closed at M5, below). M3 (`gl_heap.c` → `quake-render::heap`, [ADR-015](adr/ADR-015-renderer-port-then-modernize.md)): the device-memory suballocator is a backend-generic Rust crate module (segments, pages, small-allocation buckets, size-class bitfields, dedicated allocations, ported bit for bit), the `GL_Heap*` ABI lives in `quake-capi/src/gl_heap.rs` over the C `R_AllocateVulkanMemory`/`R_FreeVulkanMemory`/`GL_SetObjectName` seams, `gl_heap.c` is swapped for `gl_heap_glue.c` (the `_DEBUG` `test_gl_heap` command only) under `-Duse_rust_render`, `ash` 0.38 (no default features, no transitives) is adopted in ADR-003 for the `VkDeviceMemory` handle, and `quake-types::render` carries the `glheapstats_t`/`vulkan_memory_t` mirrors checked by a `render_abi` probe (ADR-011 amendment). Verified on the Windows dev box: the `gl_heap` differential (10 000 proptest traces plus the self-test's own trace shape, comparing offsets, memory handles and every `glheapstats_t` field against the C oracle), `test_gl_heap` in a debug mixed build, headless corpus 11/11 identical, and the render corpus with identical `-renderhash` chains, SSIM 1.0 and validation clean; the plan records that the C `TestHeapCleanState` invariant only holds for segments of at least 128 pages and that one unreproduced `-renderhash` outlier joined RA14 (closed at M5). M4 (2026-09-07): `gl_texmgr.c` ported to `quake-render::texmgr` behind `use_rust_render` (`Quake/gl_texmgr_glue.c` keeps `TexMgr_Init`, the palettes/well-known pointers/cvars, the stb downsampler and the `vulkan_globals` accessor); op-trace differential bit-exact, render corpus `-renderhash` identical with validation clean, `gltexture_t` ABI rows green. The STOP A review was extended to M3 (NON-BLOCKING). M5 (2026-09-10): `gl_rmisc.c` ported to `quake-render::rmisc` (device memory, buffers, staging and dynamic ring buffers, descriptor layouts/pool, samplers, the 61 shader modules and every pipeline family over `ash`, generic over an `Engine` trait) behind `use_rust_render`; `vulkan_globals` and the allocation counters are Rust-owned with the C layout exported from `quake-capi/src/gl_rmisc.rs` ([ADR-007](adr/ADR-007-singleton-ownership.md) row closed for storage, [ADR-011](adr/ADR-011-ffi-tooling.md) `VulkanGlobals` mirror checked by 152 compile-time asserts in `Quake/gl_rmisc_glue.c`, which also keeps `R_Init`, the cvar callbacks, player skins, worldspawn parsing and `vkmemstats` in C); the M3 heap backend now calls the Rust allocator and the M4 texture manager reads `vulkan_globals` directly. `-renderhash` gained the pipeline preamble (`P` line: creation-order chain over pipeline names, 330 pipelines, identical C/Rust), and RA14 is root-caused and closed: the harness never cleared its pipeline-handle table on `R_DestroyPipelines`, so the frame-1 `vid_restart` recreation overflowed the 512-entry table (660 registrations) and which used pipelines lost their name depended on driver handle reuse (deterministic in C, run-to-run variable in the Rust build); a `Harness_RenderPipelinesDestroyed` hook clears the table in both builds. Also found: `r_brush.c` passes `NULL` out-pointers to `R_StorageAllocate`, so the Rust allocators null-check like the C. Verified on the Windows dev box: render corpus C-vs-Rust and C-renderer-vs-Rust identical `-renderhash` on all five entries with `--validation`, SSIM 1.0, zero validation messages, six-run Rust stability, headless corpus 11/11 identical, ABI probe and signature checks green. M6 (2026-09-11): the Vulkan half of `gl_vidsdl.c` ported to `quake-render::vid` over `ash` ([ADR-015](adr/ADR-015-renderer-port-then-modernize.md)): instance/physical-device/device creation with the extension and feature negotiation (ray-query bundle, `VK_KHR_present_id2`/`present_wait2`, `VK_EXT_full_screen_exclusive` on Windows, the debug-utils messenger), the swap chain and its `vid_vsync`/`vid_maxframelatency` handling, render passes, frame buffers, the per-frame images and descriptor sets, `GL_BeginRendering`/`GL_AcquireNextSwapChainImage`/`GL_EndRendering` with the begin/end-rendering tasks, screenshot readback and the GPU-time counters, `GL_WaitForDeviceIdle` and `GL_SetObjectName` (the M3-M5 C seams retire); `quake-capi/src/gl_vidsdl.rs` exports the `glquake.h` block and owns `prev_end_rendering_task`; `Quake/gl_vidsdl_glue.c` keeps the SDL half verbatim (window, mode list, `vid_*` cvars, `VID_Init`/`VID_Restart`, video menu, `SCR_ScreenShot_f`) plus the `VID_Glue_*` accessors the Rust half reads SDL state through, so the `SDL_Vulkan_GetVkGetInstanceProcAddr` loading path and the macOS MoltenVK link are unchanged and SDL2/SDL3 both stay. ash 0.38 lacks the `present_id2`/`present_wait2` types, so those are declared by hand in `quake-render::vid` (plan RA8 closed). Verified on the Windows dev box: render corpus C-vs-Rust and C-renderer-vs-Rust identical `-renderhash` on all five entries with `--validation`, SSIM 1.0, zero validation messages, `timedemo` within threshold (60.0 vs 60.3 fps), the `_DEBUG` leg SSIM 1.0 and validation clean, headless corpus 11/11 identical, signature checks green; the SDL2 glue branches compile only in Linux CI. STOP B review (M4-M6) done, NON-BLOCKING. Next: M7 (draw layer).
