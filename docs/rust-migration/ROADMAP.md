...
- Deferred: the deletion wave + `c-reference/phase1` tag (blocked on the PLAN §3 decision: either GNU-ABI Rust comes in scope — `cargo --target` in `cargo_build.py`, a cargo step in the Makefiles — or those legs are retired; at deletion time the C files move to `rust/quake-ctest/csrc/` as frozen differential references); `%g`/`%e` formatter conversions (no engine user in the ported set; panics if reached); linux/windows registered-tier goldens (unchanged from Phase 0); c2rust oracle translation of `mathlib.c` (no toolchain on the dev host; the exhaustive differential FFI tests substitute); Miri CI job (run locally on the pure crates instead).
