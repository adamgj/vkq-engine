...
- **Ownership inversion** happens late (Phase 9): `main()` moves to Rust (`quake-platform::main_sdl`, exported from the `quake_rs` staticlib through `quake-capi`) while Meson keeps linking the executable -- the Phase 9 outcome of "decide by remnant size" is Meson retained, thin: the C remnant is still >150k lines, so no `libquake_c.a` is materialised and `quake-host` stays a library crate. Meson remains the build orchestrator until the C remnant is small enough for the `cc` crate, and cargo remains the Rust compiler driver throughout.
...
├── quake-host/            # host-side types (HostError); main() lives in quake-platform::main_sdl since Phase 9
