...
- **The loop consumes the status as a `Result`.** `host_frame` maps the
  ...
  the loop just goes round again. A status `Host_Guard` does not define has
...
- **`setjmp`/`longjmp` is not yet deleted.** The Rust-main frame path has no
  ...
  `USE_RUST_PLATFORM` -- but the trampoline pair, the two `jmp_buf`s and the
  ...
  / `gl_screen_glue.c` / `quakedef.h` sites convert at Phase 10, where
