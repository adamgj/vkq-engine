...
| `vulkan_globals` and the `num_vulkan_*`/`total_*_vulkan_allocation_size` counters | P8 M5 | P8 M5-M9: Rust-owned from M5 (`#[no_mangle] pub static mut vulkan_globals: VulkanGlobals` and the `AtomicU32`/`AtomicU64` counters in `rust/quake-capi/src/gl_rmisc.rs`); C reads and writes them through the `glquake.h` declarations as before, so the layout is the ADR-011 `VulkanGlobals` mirror checked by the `COMPILE_TIME_ASSERT` block in `Quake/gl_rmisc_glue.c` (152 asserts against the real headers, size differs under `_DEBUG` ⇔ cargo `engine-debug`). The Rust port itself never forms a `&`/`&mut VulkanGlobals` while C or a task worker can touch the struct: `rmisc::Ctx::vg` is a raw-pointer `VgPtr` with single-field accesses, `device_idle` (the one field workers write) is stored atomically on the Rust side, and the few whole-struct references are scoped to callback-free main-thread regions | P8 (**closed at M5** for storage; the `gl_vidsdl.c` writers (device/swap-chain/render-pass/frame-buffer fields) are Rust since M6 — `quake-capi/src/gl_vidsdl.rs` also owns `prev_end_rendering_task` (`glquake.h:701`, written by `gl_screen.c`, main thread only); the `gl_rmain.c` frame fields retire with M7-M9) |
...
| ambient `qcvm` + `pr_global_struct` (the two pointers) | P7 | P6–P7: the two pointers stay C (`Quake/pr_edict_load_glue.c:49-50` under `-Duse_rust_progs`, `Quake/pr_edict_load.c:34-35` otherwise) because 14 files outside the progs sources dereference them; the VM *logic* is Rust from P6 M3, reading the ambient global exactly once per boundary entry (ADR-008) | P7 (**closed at M9g**: `rust/quake-capi/src/progs_load.rs` now defines both as `#[no_mangle] pub static mut` under the `progs` feature, which tracks `-Duse_rust_progs` exactly. `Quake/pr_edict_load_glue.c` keeps `PR_SwitchQCVM` and writes that storage but defines none of it; `Quake/pr_edict_load.c` keeps its own copies for the oracle leg, and the two files are never compiled into the same binary. `progs.h:433-435` keeps both declarations, so all 14 dereferencing files are unchanged) |
...
Five rows name a Phase 7 milestone in their "Rust-owned from" column; the milestone letters index `docs/ai/plans/rust-conversion-phase-7.md`. **M6** (`sv`/`svs`), **M7** (`cl`/`cls`), **M9d** (the edict array + string table), **M9e** (the net message globals) and **M9g** (the ambient `qcvm`/`pr_global_struct` pointer pair) have all landed, so **every Phase 7 window is closed** and the phase-exit condition on this ADR is met. The original single "qcvm + edicts" row was split at M9d because its two halves close at different points — the edict-array half at the arena flip, the pointer half only once the cbindgen surface can carry Rust-owned storage.
