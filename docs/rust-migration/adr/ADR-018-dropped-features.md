...
4. **The GNU Makefiles and their CI legs** (`Quake/Makefile`, `Quake/Makefile.w64`, `Quake/Makefile.w64a`, `.github/workflows/build-mingw.yml`, `.github/workflows/build-msys2-clangarm64.yml`) -- *added 2026-09-19 by the Phase 9 deletion PR.* They were pure-C builds with no Rust path (PLAN §3 left their retirement to the maintainers' option), and the C sources they compiled are deleted by that PR, so they cannot survive it. Meson is the sole build on every platform; GNU-ABI Rust linkage (MinGW/clangarm64) remains out of scope, so those toolchains are no longer supported targets.
